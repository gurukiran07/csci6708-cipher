...
digraph_t make_digraphs(std::string plain_text){
    plain_text.erase(std::remove_if(plain_text.begin(), plain_text.end(), is_removable_char), plain_text.end());
    size_t idx = 0;
    digraph_t digraphs{};
    ...
    while( idx < (plain_text.length() - 1) ){
        ...
        if ( first != second ){
            ...
            digraphs.push_back({first, first == REPEATED_CHAR_Q ? REPEATED_CHAR_X : REPEATED_CHAR_Q});
            ++idx;
        }
    }
...
std::string encrypt(std::string plain_text, matrix_t matrix){
    ...
    for(auto& digraph: digraphs){
        ...
        // std::cout << digraph.first << " index: " << idx[0].first << " " << idx[0].second << "\n";
        // std::cout << digraph.second << " index: " << idx[1].first << " " << idx[1].second << "\n";
...
std::string decrypt(std::string cipher_text, matrix_t matrix){
    cipher_text.erase(std::remove_if(cipher_text.begin(), cipher_text.end(), is_removable_char), cipher_text.end());
    std::string cipher = "";
    size_t i = 0;

    while (i < cipher_text.length()){
        auto idx = idx_pair({cipher_text[i], cipher_text[i + 1]}, matrix);
        // std::cout << cipher_text[i] << " index: " << idx[0].first << " " << idx[0].second << "\n";
        // std::cout << cipher_text[i + 1] << " index: " << idx[1].first << " " << idx[1].second << "\n";

        if (idx[0].first == idx[1].first){
            auto cdx = (idx[0].second - 1) < 0 ? (5 + (idx[0].second - 1)) : (idx[0].second - 1);
            auto cdx1 = (idx[1].second - 1) < 0 ? (5 + (idx[1].second - 1)) : (idx[1].second - 1);
            cipher += matrix[idx[0].first][cdx];
            cipher += matrix[idx[1].first][cdx1];
        }
        else if (idx[0].second == idx[1].second){
            auto cdx = (idx[0].first - 1) < 0 ? 5 + (idx[0].first - 1): (idx[0].first - 1);
            auto cdx1 = (idx[1].first - 1) < 0 ? 5 + (idx[1].first - 1): (idx[1].first - 1);
